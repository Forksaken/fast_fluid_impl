...
struct VectorField {
    ...
    Fixed &get(int x, int y, int dx, int dy) {
        size_t i = distance(deltas.begin(), find(deltas.begin(), deltas.end(), pair(dx, dy)));
...
int main() {
    ...
    for (size_t i = 0; i < T; ++i) {
        ...
        #pragma omp parallel for collapse(2) schedule(static) reduction(+:total_delta_p)
        for (size_t x = 0; x < N; ++x) {
            for (size_t y = 0; y < M; ++y) {
                if (field[x][y] == '#')
                    continue;
        ...
        do {
            UT += 2;
            prop = false;
            #pragma omp parallel for collapse(2) schedule(static) reduction(||:prop)
            for (size_t x = 0; x < N; ++x) {
                for (size_t y = 0; y < M; ++y) {
            ...
            for (size_t y = 0; y < M; ++y) {
                ...
                for (auto [dx, dy] : deltas) {
                    ...
                    if (old_v > 0) {
                        ...
                        if (field[x][y] == '.')
                            force = force * Fixed::from_raw(0.8 * (1 << 16));
                        if (field[x + dx][y + dy] == '#') {
                            p[x][y] += force / dirs[x][y];
                            total_delta_p += force / dirs[x][y];
                        } else {
